...
#include "MNVM_interface.h"
#include "MNVM_private.h"
#include "MNVM_configure.h"
...
void HNVM_voidWrite(u16 ARG_u16Address,u8 ARG_u8Data)
{
			...
			if(MTWI_STATUS_MASTER_START_CONDITION == MTWI_status	MTWI_twistatusCheckStatus())
			{
				MTWI_voidLoadTWDR((( L_u8SlaveAddress << 1) & (0xFE)) | (0));
				MTWI_voidClearFlag();
				MTWI_voidFlagPolling();
				if(MTWI_STATUS_MASTER_TX_SLA_W_ACK == MTWI_status	MTWI_twistatusCheckStatus())
				{
					MTWI_voidLoadTWDR((u8)ARG_u16Address);
					MTWI_voidClearFlag();
				    MTWI_voidFlagPolling();
					if(MTWI_STATUS_MASTER_TX_DATA_ACK == MTWI_status	MTWI_twistatusCheckStatus())
					{
						if(MTWI_STATUS_MASTER_TX_DATA_ACK == MTWI_status	MTWI_twistatusCheckStatus())
					  {
						  MTWI_voidLoadTWDR(u8 ARG_u8Data);
					      MTWI_voidClearFlag();
				          MTWI_voidFlagPolling();
						if(MTWI_STATUS_MASTER_TX_DATA_ACK == MTWI_status	MTWI_twistatusCheckStatus())
					    {
			...
			if(MTWI_STATUS_MASTER_START_CONDITION == MTWI_status	MTWI_twistatusCheckStatus())
			{
				...
				if(MTWI_STATUS_MASTER_TX_SLA_W_ACK == MTWI_status	MTWI_twistatusCheckStatus())
				{
					...
					if(MTWI_STATUS_MASTER_TX_DATA_ACK == MTWI_status	MTWI_twistatusCheckStatus())
					{
						 MTWI_voidSendStartCondition();
			             MTWI_voidFlagPolling();
						if(MTWI_STATUS_MASTER_RSTART_CONDITION  == MTWI_status	MTWI_twistatusCheckStatus())
		            	{
		            		MTWI_voidLoadTWDR((( L_u8SlaveAddress << 1) & (0xFE)) | (1));
		            		MTWI_voidClearFlag();
		            		MTWI_voidFlagPolling();
							 if(MTWI_STATUS_MASTER_RX_SLA_R_ACK == MTWI_status	MTWI_twistatusCheckStatus())
				             {
				            	
				            	MTWI_voidClearFlag();
				                MTWI_voidFlagPolling();
								if(MTWI_STATUS_MASTER_RX_DATA_NACK == MTWI_status	MTWI_twistatusCheckStatus())   
